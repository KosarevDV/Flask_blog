...
@posts.route("/allpost")
# переход к списку постов блога возможен только после прохождения пользователем авторизации.
@login_required
def allpost():
...
def new_post():
    ...
    return render_template('create_post.html', title='Новый пост', form=form, legend='Новый пост', image_file=picture_name)
...
def update_post(post_id):
    post = Post.query.get_or_404(post_id)
    # доступ к редактированию поста будет только у автора
    ...
    return render_template('create_post.html', title='Обновление поста', form=form,
                           legend='Обновление поста', image_file=image_file)
...
def like_post(post_id):
    # Получаем объект поста через модель и ORM по идентификатору
    post = Post.query.get_or_404(post_id)

    if post.author == current_user:
        flash('Вы не можете поставить лайк самому себе', 'warning')
    #  если уже есть «лайк», оставленный текущим пользователем - значит отменяем его
